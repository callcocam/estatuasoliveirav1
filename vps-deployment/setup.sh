...
clear
step "VPS Deployment — estatuasoliveira (production)"
...
ask PROJECT_NAME "Nome do projeto" "${PROJECT_NAME:-estatuasoliveira}"
            ...
            if [[ "${_deploy_ssh_ok}" == "true" ]]; then
                ${SSH_DEPLOY} "${DEPLOY_USER}@${VPS_HOST}" "
                    ...
                    if [ -d \"/opt/\${APP_SLUG}/\${PROJECT_NAME}\" ] && [ -f \"/opt/\${APP_SLUG}/\${PROJECT_NAME}/docker-compose.yml\" ]; then
                        cd \"/opt/\${APP_SLUG}/\${PROJECT_NAME}\" && docker compose -p \"estatuasoliveira-\${APP_SLUG}\" down --remove-orphans || true
                    fi
