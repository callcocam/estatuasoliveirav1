...
PROJECT_NAME="${PROJECT_NAME:-estatuasoliveira}"
...
if [[ "${START_SERVICES}" == "true" ]]; then
    ...
    echo "[app] Baixando imagem da aplicação estatuasoliveira-${APP_SLUG}..."
    echo "[app] (Se for o primeiro deploy a imagem pode ainda não existir no registry — tudo bem)"
    if (cd "${APP_DIR}" && docker compose -p "estatuasoliveira-${APP_SLUG}" pull 2>&1); then
        (cd "${APP_DIR}" && docker compose -p "estatuasoliveira-${APP_SLUG}" up -d)
        echo "[app] estatuasoliveira-${APP_SLUG} iniciado com sucesso"
    else
