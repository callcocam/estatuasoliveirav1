...
Provisionamento e deploy do `estatuasoliveira` no VPS, ambiente production. Banco com conexão única.
...
docker compose -p estatuasoliveira-<APP_SLUG> ps
docker compose -p estatuasoliveira-<APP_SLUG> exec -T app sh -lc 'curl -fsS http://127.0.0.1/up >/dev/null && echo OK'
```
...
```bash
docker compose -p estatuasoliveira-<APP_SLUG> exec -T app php artisan migrate --force
```
...
```bash
docker compose -p estatuasoliveira-<APP_SLUG> exec -T app sh -lc '
...
docker compose -p estatuasoliveira-<APP_SLUG> up -d --force-recreate
```
...
docker compose -p estatuasoliveira-<APP_SLUG> exec -T app php artisan optimize:clear
docker compose -p estatuasoliveira-<APP_SLUG> up -d --force-recreate
```
