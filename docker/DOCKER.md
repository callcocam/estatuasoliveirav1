...
> O nome do projeto é o token **`estatuasoliveira`** em todos os arquivos. O `init.sh` na raiz do scaffold
> substitui `estatuasoliveira` pelo nome real do seu projeto — rode-o antes de usar.
...
Rode `./init.sh` (substitui `estatuasoliveira` por todo lado), ou troque manualmente em `docker-compose.yml`:

| Superfície | Valor |
|---------|------------------|
| compose | `name: estatuasoliveira` |
| nginx   | `traefik.http.routers.estatuasoliveira-web.rule=Host(\`estatuasoliveira.localhost\`)` |
| pgadmin | `traefik.http.routers.estatuasoliveira-pgadmin.rule=Host(\`pgadmin.estatuasoliveira.localhost\`)` |
...
```env
APP_URL=http://estatuasoliveira.localhost
DB_CONNECTION=pgsql
DB_HOST=postgres
DB_PORT=5432
DB_DATABASE=estatuasoliveira
DB_USERNAME=estatuasoliveira
DB_PASSWORD=estatuasoliveira
REDIS_HOST=redis
REDIS_PORT=6379
MAIL_MAILER=smtp
MAIL_HOST=mailpit
