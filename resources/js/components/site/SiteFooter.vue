...
import { about, contact, gallery, home, login, terms } from '@/routes';
...
<template>
    <footer
        class="border-t border-site-outline-variant bg-site-surface-container-low"
    >
        <div
            class="mx-auto grid max-w-6xl gap-10 px-4 py-14 md:grid-cols-3 md:px-6"
        >
            <div>
                <p class="font-display text-xl text-site-primary">
                    {{ company.name }}
                </p>
                <p
                    class="mt-3 max-w-xs text-sm leading-relaxed text-site-on-surface-variant"
                >
            ...
            <nav :aria-label="t('app.site.footer.navigation')">
                <p
                    class="text-xs font-semibold tracking-widest text-site-on-surface uppercase"
                >
        ...
        <div class="border-t border-site-outline-variant py-6">
            <div
                class="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-2 px-4 text-xs text-site-on-surface-variant md:px-6"
            >
                <p>
                    © {{ year }} {{ company.name }}.
                    {{ t('app.site.footer.rights') }}
                </p>
                <Link
                    :href="login()"
                    class="transition-colors hover:text-site-primary"
                >
                    {{ t('app.site.auth.customer_area') }}
                </Link>
            </div>
        </div>
    </footer>
