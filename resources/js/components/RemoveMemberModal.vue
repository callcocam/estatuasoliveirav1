...
<template>
    <Dialog :open="props.open" @update:open="emit('update:open', $event)">
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{{
                    t('app.teams.modals.remove.title')
                }}</DialogTitle>
                <DialogDescription>
                    {{ t('app.teams.modals.remove.description_prefix') }}
                    <strong>{{ props.member?.name }}</strong>
                    {{ t('app.teams.modals.remove.description_suffix') }}
                </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter class="gap-2">
                ...
                <Button
                    ...
                    @click="removeMember"
                >
                    {{ t('app.teams.modals.remove.submit') }}
                </Button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
