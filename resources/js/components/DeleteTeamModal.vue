    ...
    <Dialog :open="props.open" @update:open="handleOpenChange">
        <DialogContent>
            ...
            >
                <DialogHeader>
                    <DialogTitle>{{
                        t('app.teams.modals.delete.title')
                    }}</DialogTitle>
                    <DialogDescription>
                        {{ t('app.teams.modals.delete.description_prefix') }}
                        <strong>"{{ props.team.name }}"</strong
                        >{{ t('app.teams.modals.delete.description_suffix') }}
                    </DialogDescription>
                </DialogHeader>
                ...
                <div class="space-y-4 py-4">
                    <div class="grid gap-2">
                        <Label for="confirmation-name">
                            {{
                                t(
                                    'app.teams.modals.delete.confirm_label_prefix',
                                )
                            }}
                            <strong>"{{ props.team.name }}"</strong>
                            {{
                                t(
                                    'app.teams.modals.delete.confirm_label_suffix',
                                )
                            }}
                        </Label>
                        <Input
                            ...
                            :placeholder="
                                t('app.teams.modals.delete.name_placeholder')
                            "
                            autocomplete="off"
                        />
                ...
                <DialogFooter class="gap-2">
                    ...
                    >
                        {{ t('app.teams.modals.delete.submit') }}
                    </Button>
                </DialogFooter>
            </Form>
        </DialogContent>
