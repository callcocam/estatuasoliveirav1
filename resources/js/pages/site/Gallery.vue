...
<template>
    <SiteLayout :title="t('app.site.meta.gallery_title')" :description="t('app.site.gallery.subtitle')">
        <div class="mx-auto max-w-6xl px-4 py-14 md:px-6 md:py-20">
            <SectionHeading :title="t('app.site.gallery.title')" :subtitle="t('app.site.gallery.subtitle')" align="center" />

            <InfiniteScroll v-if="images.data.length" data="images" :buffer="400">
                <div class="columns-2 gap-4 md:columns-3 lg:columns-4 [&>button]:mb-4">
                    ...
                    >
                        <img
                            :src="image.url"
                            :alt="image.productName ?? t('app.site.gallery.title')"
                            loading="lazy"
                ...
                <template #loading>
                    <div class="mt-4 grid grid-cols-2 gap-4 md:grid-cols-4">
                        <div v-for="index in 4" :key="index" class="aspect-square animate-pulse rounded-site-card bg-site-surface-container" />
                    </div>
                </template>
            </InfiniteScroll>

            <p v-else class="py-20 text-center text-site-on-surface-variant">{{ t('app.site.gallery.empty') }}</p>
        </div>
        ...
        >
            ...
            <figure class="max-h-full max-w-4xl">
                <img
                    ...
                    :alt="lightboxImage.productName ?? t('app.site.gallery.title')"
                    class="max-h-[80vh] w-auto rounded-site-card object-contain"
                />
                <figcaption v-if="lightboxImage.productName" class="mt-4 flex items-center justify-between gap-4 text-site-inverse-on-surface">
                    <span class="font-display text-lg">{{ lightboxImage.productName }}</span>
                    <Link
